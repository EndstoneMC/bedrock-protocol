...
// name-coded, as EAS::BoolAttributeData dumps it. The golden takes the DSL's spelling;
// BDS lowercases before the lookup, so the casing is not load-bearing on the wire. The
// easing name is the same patch: EasingType is name-coded and BDS spells it snake_case, so
// the member's own spelling reaches the wire and lowercases back onto it.
const std::string golden_v975 = bytes({
    0x02, 0x03, 0x77, 0x65, 0x74, 0x00, 0x01, 0x04, 0x74, 0x65, 0x6d, 0x70,
    0x00, 0x00, 0x01, 0x08, 0x4f, 0x56, 0x45, 0x52, 0x52, 0x49, 0x44, 0x45,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x4c, 0x49,
    0x4e, 0x45, 0x41, 0x52,
});
...
const std::string golden_v1001 = bytes({
    ...
    0x00, 0x00, 0x01, 0x08, 0x4f, 0x56, 0x45, 0x52, 0x52, 0x49, 0x44, 0x45,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x4c, 0x49,
    0x4e, 0x45, 0x41, 0x52, 0x00, 0x00, 0x00, 0x00, 0x00,
});
