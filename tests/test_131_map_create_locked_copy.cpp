...
TEST_CASE("packet id is 131")
{
    ...
    STATIC_REQUIRE_FALSE(bp::has_packet_v<1001, 131>);
...
TEST_CASE("map-create-locked-copy round-trips against the golden")
{
    ...
    packet.original_map_id = static_cast<bp::ActorUniqueID>(300);
    packet.new_map_id = static_cast<bp::ActorUniqueID>(-5);
    REQUIRE(encode(packet) == golden);

    const auto back = decode<bp::MapCreateLockedCopyPacket_<2168>>(golden);
    REQUIRE(back.original_map_id == static_cast<bp::ActorUniqueID>(300));
    REQUIRE(back.new_map_id == static_cast<bp::ActorUniqueID>(-5));
}

TEST_CASE("both locked-copy map ids are 64-bit zigzag varints")
{
    bp::MapCreateLockedCopyPacket_<2168> packet;
    packet.original_map_id = static_cast<bp::ActorUniqueID>(-8589934592);
    packet.new_map_id = static_cast<bp::ActorUniqueID>(300);
    const auto wire = encode(packet);
    REQUIRE(wire.size() == 7);

    const auto back = decode<bp::MapCreateLockedCopyPacket_<2168>>(wire);
    REQUIRE(back.original_map_id == static_cast<bp::ActorUniqueID>(-8589934592));
    REQUIRE(back.new_map_id == static_cast<bp::ActorUniqueID>(300));
}
