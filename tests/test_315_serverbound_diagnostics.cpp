...
// for only 7 of 565 files, so the framing could still be two bytes. No r26_u4 headers
// exist to break the tie, so the leading 0x01 below is patched in and the rest is the
// codec's own output.
// generated by CloudburstMC Bedrock_v2168:
...
{
    using Counter = typename decltype(packet.memory_category_values)::value_type;
    ...
    packet.memory_category_values.push_back({.category = Category::ORE_UI, .current_bytes = 1024});
...
TEST_CASE("serverbound-diagnostics round-trips against the golden")
{
    ...
    REQUIRE(back.memory_category_values[0].category == bp::MemoryCategory_<1001>::ORE_UI);
...
TEST_CASE("serverbound-diagnostics v2168 form round-trips against the golden")
{
    ...
    REQUIRE(back.memory_category_values[0].category == bp::MemoryCategory_<2168>::ORE_UI);
...
TEST_CASE("a v1001 category byte reads as another category at v2168")
{
    ...
    const auto category = back.memory_category_values[0].category;
