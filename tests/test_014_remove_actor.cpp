...
TEST_CASE("packet id is 14")
{
    ...
    STATIC_REQUIRE(bp::has_packet_v<1001, 14>);
...
TEST_CASE("remove-actor round-trips against the golden")
{
    ...
    const auto back = decode<bp::RemoveActorPacket>(golden);
...
TEST_CASE("a negative unique id zigzags instead of sign-extending")
{
    ...
    REQUIRE(wire.size() == 1);
    REQUIRE(decode<bp::RemoveActorPacket>(wire).entity_id == bp::ActorUniqueID{-1});
}
...
TEST_CASE("a unique id above 127 is a varint, not a fixed 64-bit field")
{
    ...
    const auto wire = encode(packet);
    REQUIRE(wire.size() == 2);
    REQUIRE(decode<bp::RemoveActorPacket>(wire).entity_id == bp::ActorUniqueID{300});
}
