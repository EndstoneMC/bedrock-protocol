...
TEST_CASE("InventorySlotPacket v944: Container is non-optional, items are legacy descriptors")
{
    ...
    pkt.inventory_id = 0;
    pkt.slot = 5;
    pkt.full_container_name_v729.name = bp::ContainerEnumName::InventoryContainer;
    pkt.full_container_name_v729.dynamic_id = std::nullopt;
    // air storage_item + air item -- the legacy descriptor's id == 0 shortcut omits the body.
    pkt.storage_item_v748.id = 0;
    pkt.item.id = 0;
    ...
    REQUIRE(rt.has_value());
    REQUIRE(rt->full_container_name_v729.name == bp::ContainerEnumName::InventoryContainer);
    REQUIRE_FALSE(rt->full_container_name_v729.dynamic_id.has_value());
    REQUIRE(rt->storage_item_v748.id == 0);
