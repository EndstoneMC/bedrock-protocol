...
from .names import camel, requires_clause, snapshot_namespace
...
class FileGenerator:
    ...
    def render_header(self, latest_version: int) -> str:
        """The `.hpp`: type definitions, the `_<V>` version selector, and
        declaration-only `Serializer<T>` specializations."""
        p = Printer()
        self._emit_pragma(p)
    ...
    def render_source(self) -> str:
        """The `.cpp`: out-of-line `Serializer<T>` bodies, compiled once into
        the static library. Consumers see only the header's declarations."""
        p = Printer()
        p(f'#include "{self._file.stem}.hpp"')
        p()
        self._emit_namespace_open(p)
    ...
    def _emit_includes(self, p: Printer) -> None:
        ...
        if self._ctx.string_coded_enums:
            stdlib.insert(1, "<cctype>")
            stdlib.insert(7, "<unordered_map>")
        if uses_bitset:
    ...
    def _emit_version_include(self, p: Printer) -> None:
        """Files with versioned types spell the `_<V>` selector over the
        generated `ProtocolVersion` enum, so pull in its header."""
        ...
        for name, f in self._file_set.files.items():
            ...
            if any(e.name == "ProtocolVersion" for e in f.enums):
                if name in self._file.imports:
                    return  # already emitted by _emit_dep_includes
                path = name.replace(".", "/") + ".hpp"
                p()
                p(f'#include "{path}"')
                return

    def _version_enum(self) -> Enum | None:
        """The DSL-owned protocol-version enum, if the file set declares one.
        Its members drive the `_<V>` selector's parameter type."""
    ...
    def _emit_traits(self, p: Printer) -> None:
        ...
        for name in self._versioned_names():
            p(f"template <{vparam} V> using {name}_ = typename detail::{name}_<{varg}>::type;")
    ...
    def _emit_serializers(self, p: Printer, mode: str) -> None:
        ...
        for name in self._resolved.declaration_order:
            ...
            if isinstance(t, Enum):
                ...
                if not fresh:
                    p()
                    gen.emit_for_enum(p, t, None, mode=mode)
                else:
                    for s in fresh:
                        assert s.enum is not None
                        p()
                        gen.emit_for_enum(p, s.enum, s.lo, mode=mode)
            else:
                ...
                if fresh and _has_versioned_nested(t):
                    ...
                    for ns in _walk_nested_structs(t, ""):
                        pos = len(p.lines)
                        p()
                        if not gen.emit_for_struct(p, ns, None, mode=mode):
                            del p.lines[pos:]
                ...
                if not fresh:
                    pos = len(p.lines)
                    p()
                    if not gen.emit_for_struct(p, t, None, mode=mode):
                        del p.lines[pos:]
                else:
                    for s in fresh:
                        assert s.struct is not None
                        pos = len(p.lines)
                        p()
                        if not gen.emit_for_struct(p, t, s.struct, snapshot=s.lo, mode=mode):
                            del p.lines[pos:]
        for a in self._file.type_aliases:
            if isinstance(a.target, VariantType):
                p()
                gen.emit_for_variant_alias(p, a, mode=mode)
    ...
    def _emit_latest_aliases(self, p: Printer, latest_version: int) -> None:
        ...
        for name in names:
            ...
            if until is not None and latest_version >= until:
                ...
                continue
            p(f"using {name} = {name}_<{latest_arg}>;")
