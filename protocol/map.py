...
class ClientboundMapItemDataPacket:
    # BDS: ClientboundMapItemDataPacket::Type bitfield. The wire is a single
    # uvarint32 -- TextureUpdate=2, DecorationUpdate=4, Creation=8 -- OR'd
    # together to gate the four payload blocks below. The DSL surface keeps it as
    # the raw uvarint32 so per-bit `when=` gating can test it without needing an
    # IntFlag spelling.
    ...
    # COMPILER_EXTENSION_NEEDED: the wire `type` discriminator is COMPUTED from
    # whether each payload block is populated (CloudburstMC sets bit-3 of `type`
    # iff `trackedEntityIds` is non-empty, etc). The DSL surface treats `type` as
    # a raw uvarint32 the caller pins, so a caller has to mirror which payload
    # blocks they populate manually; the DSL has no spelling for "this enum bit
    # is true iff field X is non-empty" on serialize.
    map_ids: list[ActorUniqueID] = field(when=lambda p: p.type & 0x8 != 0)

    # FLAG_ALL = 0xE. Scale is present whenever any of TextureUpdate /
    # DecorationUpdate / Creation is set.
    scale: int8 = field(when=lambda p: p.type & 0xE != 0)
    ...
    # FLAG_TEXTURE_UPDATE = 0x2. width/height/x_offset/y_offset followed by a
    # uvarint32-prefixed list of packed-int colours (one per pixel).
    with field(when=lambda p: p.type & 0x2 != 0):
        width: varint32
        height: varint32
        x_offset: varint32
        y_offset: varint32
