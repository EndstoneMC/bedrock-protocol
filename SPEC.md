...
- **No `from __future__ import annotations`** (`.claude/rules/style.md`). The compiler reads
...
the wire references show no fields, the body stays empty (`.claude/rules/protocol.md`).
...
against that body (`.claude/rules/tests.md`).
...
A single gated field uses the inline form above (`.claude/rules/protocol.md`). For two
...
Three forms, picked by what the member needs (`.claude/rules/protocol.md`):
...
floor) is `.claude/rules/protocol.md`. This document only describes the DSL forms that
express the result.

## 9. Docstrings

A `@packet` / `@type` docstring is the Description from
`Mojang/bedrock-protocol-docs`, carried across faithfully (`.claude/rules/protocol.md`).
...
- **Golden round-trip per packet** (`.claude/rules/tests.md`). Every packet gets a
  ...
  that embeds them.
- **Tests as usage examples** (`.claude/rules/tests.md`). Keep each public API shape
...
- ASCII only, no semicolon clause-splices (`.claude/rules/style.md`).
- Resolve every symbol name through bedrock-headers, then EndstoneMC
  protocol-docs, then the Mojang `.dot` files, in that order (`.claude/rules/protocol.md`).
- Source docstrings from `Mojang/bedrock-protocol-docs` (`.claude/rules/protocol.md`).
- Cross-check field / member version history against gophertunnel and
  CloudburstMC before gating (`.claude/rules/protocol.md`).
- Add a gophertunnel-generated golden test for every new packet (`.claude/rules/tests.md`).
...
- When protocol-docs disagrees with gophertunnel and CloudburstMC on a field's
  ...
  one (`.claude/rules/protocol.md`).
- When no source names a symbol, raise it rather than inventing a name (`.claude/rules/protocol.md`).
- When a packet has no gophertunnel equivalent for a golden, raise it rather
  than hand-writing the bytes (`.claude/rules/tests.md`).

Never:

- Add `Co-Authored-By: Claude` lines (`.claude/rules/style.md`).
- Emit explanatory `//` comments from Jinja templates into generated headers
  (`.claude/rules/style.md`).
- Use `from __future__ import annotations` (`.claude/rules/style.md`).
- Lift a symbol name from gophertunnel or CloudburstMC -- those date and shape
  symbols, they do not name them (`.claude/rules/protocol.md`).
- Infer wire fields from a BDS header -- headers name symbols, the protocol
  references define the wire shape (`.claude/rules/protocol.md`).
